...
> [!TIP]
> **v2.7.4 重大更新！** 智能错误处理与系统稳定性重大升级，实现87007充电专享视频自动处理、任务队列持久化和失败任务智能筛选，带来零干预下载体验。详见下方"🔧 最新更新"部分。

## 🌟 项目亮点

### 🆕 v2.7.4 最新特性

- **🎯 87007智能处理** - 自动识别并处理充电专享视频，零干预体验，彻底解决重复错误问题
- **🛡️ 任务队列持久化** - 任务状态保存到数据库，程序重启后自动恢复，保证企业级稳定性
- **🔍 失败任务智能筛选** - 一键筛选所有失败任务，快速定位问题，提升故障排查效率
- **🔥 配置热重载系统** - 配置完全基于数据库，支持实时热重载，无需重启即可生效
...
#### 🎉 2025-06-16 更新 - v2.7.3 配置系统革新 🔥
- **🔥 配置热重载系统** - 配置完全基于数据库存储，使用 ArcSwap 架构实现高性能热重载
- **📝 文件名处理增强** - 增强的 filenamify 函数，自动处理全角字符（`：`→`-`，`「」`→`[]`）
- **🚀 任务队列优化** - 配置保存通过专门的任务队列处理，避免数据库锁定冲突
- **🎯 初始设置向导** - 首次启动显示友好的设置界面，简化 Cookie 配置流程
- **🔧 内存泄漏修复** - 完全移除 Box::leak 导致的内存泄漏问题
- **✅ 凭证保存修复** - 修复 credential 刷新后无法持久化的问题
- **📊 受限模式运行** - 未设置凭证时以受限模式运行，不再强制退出
