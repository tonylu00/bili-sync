...
> [!TIP]
> **v2.7.3 重大更新！** 配置系统全面升级，支持热重载；文件名处理智能化，自动转换特殊字符；新增初始设置向导，提升用户体验。详见下方"🔧 最新更新"部分。
...
- **📝 智能文件名处理** - 自动处理所有特殊字符，支持全角转换，确保跨平台兼容性
- **🚀 任务队列优化** - 配置保存通过任务队列处理，彻底解决数据库锁定问题
- **🎯 初始设置向导** - 友好的初始设置界面，轻松配置 Cookie 和基本参数
