...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        // 插入新的配置项：bangumi_use_season_structure
        // 默认值为 false，保持向后兼容性
        ...
        query.values_panic(["bangumi_use_season_structure".into(), "false".into()]);
        ...
        manager
            .exec_stmt(
                Query::delete()
                    ...
                    .and_where(Expr::col(ConfigItem::KeyName).eq("bangumi_use_season_structure"))
                    .to_owned(),
            )
            .await?;
