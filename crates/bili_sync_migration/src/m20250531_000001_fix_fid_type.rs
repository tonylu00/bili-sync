...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let has_latest_row_at = match db.execute_unprepared(
            "SELECT COUNT(*) FROM pragma_table_info('favorite') WHERE name = 'latest_row_at'"
        ).await {
            Ok(_result) => {
                // 这里我们假设如果查询成功，列就存在
        ...
        // 4. 重命名临时表
        db.execute_unprepared("ALTER TABLE favorite_temp RENAME TO favorite").await?;
