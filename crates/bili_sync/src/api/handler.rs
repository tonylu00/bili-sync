...
pub async fn update_config_internal(
    ...
    Ok(crate::api::response::UpdateConfigResponse {
        ...
        message: if should_rename && should_reset_nfo {
            format!(
                "配置更新成功，已更新字段: {}，重命名了 {} 个文件/文件夹，重置了 {} 个视频和 {} 个页面的NFO任务",
                updated_fields.join(", "),
                updated_files,
            ...
            format!(
                "配置更新成功，已更新字段: {}，重置了 {} 个视频和 {} 个页面的NFO任务",
                updated_fields.join(", "),
                resetted_nfo_videos_count,
                resetted_nfo_pages_count
            )
...
async fn rename_existing_files(
    ...
    let video_template = config.video_name.replace(['/', '\\'], "__SEP__");
    let page_template = config.page_name.replace(['/', '\\'], "__SEP__");
