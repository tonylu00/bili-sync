...
`video_detail` 参数用于控制获取视频详情信息时的并发数。这是独立于视频下载的并发控制，因为获取视频详情是轻量级的 API 请求，而视频下载是重量级的文件传输。对于低配置设备，建议将此值设置为 1-2，以避免数据库超时问题。
...
> 3. `video_detail` 对于低配置设备特别重要，建议设置较小的值（1-2）以避免数据库连接超时。