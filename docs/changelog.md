...
## v2.7.6.6 (2025-07-28)
### 实现内存数据库守护连接机制
- **彻底解决内存数据库持久化问题**: 添加守护连接确保内存数据库永不被GC回收
  - 在`MemoryDbOptimizer`中添加专门的守护连接(`keeper_connection`)
  - 守护连接在整个内存数据库生命周期内保持活跃，防止数据库被回收
  - 实现30秒间隔的心跳机制，通过`SELECT 1`查询保持连接活跃状态
  - 优化连接释放顺序，确保守护连接在清理阶段最后释放

### 连接池和稳定性优化
- **内存数据库连接池配置**: 为内存数据库添加专门的连接池设置
  - 最大连接数10个，最小连接数2个（包含守护连接）
  - 空闲超时10分钟，连接最大生命周期1小时
  - 连接获取超时10秒，优化连接管理性能
  - 禁用详细的SQLx日志，减少日志噪音

### 验证和监控增强
- **改进表存在性验证**: 优化内存数据库验证机制
  - 优先使用守护连接进行验证，提升验证可靠性
  - 增加验证日志详情，区分守护连接和业务连接验证
  - 健康检查频率从5分钟优化到2分钟，更快发现问题
  - 心跳机制提供连接状态实时监控

### 技术实现
- 守护连接与业务连接分离，确保数据库持久性不受业务逻辑影响
- 使用`tokio::spawn`异步执行心跳任务，不阻塞主要业务流程
- 连接生命周期精确管理，启动时创建、停止时按序释放
- 保留现有自动重建机制作为最后防线，双重保障系统稳定性

## v2.7.6.5 (2025-07-28)
### 修复内存数据库长时运行稳定性问题
- **解决"no such table: collection"错误**: 彻底修复长时间运行后内存数据库表丢失问题
  - 修改共享内存数据库URL，使用命名内存数据库确保连接稳定性
  - 添加内存数据库表存在性验证机制，及时发现表丢失问题
  - 实现内存数据库自动重建功能，健康检查发现问题时自动恢复
  - 优化内存数据库PRAGMA设置，使用NORMAL锁定模式避免锁定冲突

### 自动恢复机制
- **智能故障恢复**: 新增内存数据库自动重建机制
  - 周期性健康检查发现表丢失时自动触发重建流程
  - 重新配置全局内存优化器，确保数据从主数据库重新加载
  - 增加详细的恢复过程日志，便于监控恢复状态
  - 支持非内存优化模式下的连接问题诊断

### 技术改进
- 从`"sqlite:file::memory:?cache=shared"`改为`"sqlite:file:bili_sync_memory?mode=memory&cache=shared"`
- 添加`verify_memory_db_tables()`方法检查内存数据库完整性
- 优化锁定模式和缓存设置，提升内存数据库稳定性
- 增强错误恢复能力，减少需要手动重启的情况

## v2.7.6.4 (2025-07-28)
### 修复长时运行数据库连接问题
- **解决"no such table: collection"错误**: 修复长时间运行后出现的数据库连接问题
  - 增强HTTP服务器数据库连接验证机制，启动时检查关键表是否存在
  - 修复`bilibili/client.rs`中绕过全局内存优化器的连接创建问题
  - 优先使用全局内存优化器提供的数据库连接，确保连接一致性
  - 添加周期性数据库连接健康检查（每5分钟），及时发现连接问题

### 连接稳定性增强
- **数据库连接验证**: 新增连接有效性测试机制
  - HTTP服务器启动时验证数据库连接和关键表存在性
  - 支持在内存优化器连接失效时自动回退到主数据库连接
  - 增加详细的连接状态日志，便于问题诊断
  - 运行时定期检查连接健康状态，提前预警连接问题

### 技术改进
- 统一所有数据库连接的获取方式，避免连接不一致
- 增强系统在长时间运行状态下的稳定性
- 改进错误处理和日志记录，提升故障排查能力
