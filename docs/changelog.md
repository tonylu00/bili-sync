...
## v2.7.7.5.5-2 (2025-11-09)
- **新增视频过滤与下载限制**
- 后端新增视频时长与关键词过滤策略，支持配置有效范围与排除列表
- 前端支持在视频源中调整过滤规则，下载任务自动应用最新条件
- 新增下载过滤设置入口，可针对不同来源灵活启用或关闭

- **新增视频回收站功能**
- 支持列出已删除视频并手动恢复，批量操作一键还原内容
- 修复回收站按钮事件及已删除视频列表 API，确保操作反馈准确

- **推送通知系统升级**
- 新增 Bark 推送支持并统一推送 API 结构
- Web 端新增推送通知配置与事件选择界面，支持风险控制与下载失败提醒
- 下载失败与风控警报同步推送到 Bark 及现有渠道，异常即时可见

- **FFmpeg 超时管理优化**
- 新增 FFmpeg 合并超时时间配置项，根据需求自定义阈值
- 默认合并超时调整为 600 秒，解决长视频合并失败问题

- **增强 FLV 视频兼容性**
- 支持纯 FLV 流下载路径，重构相关结构与下载逻辑

- **任务调度与稳定性改进**
- 新增任务优先级处理与批量状态设置能力，提升任务队列效率
- 优化 reqwest 客户端连接池管理，增强网络故障恢复力
- 修复 SQLite 迁移脚本列增删逻辑，确保升级兼容性
- 忽略空 Bark Device Key 配置更新，避免无效写入

## v2.7.7.5.5-1 (2025-10-25)
- **优化 AI 字幕下载体验**
